...
100万件は1.32倍、300万件は1.08倍、500万件は1.14倍の差が出た。
300万件の時だけそこまで大きな差は出なかったが、基本的にはHigh Specの方が早くinsertすることができていた

<aside>
ℹ️ メモリとコア数のどちらがスピードに起因するのか疑問に思ったので、lowspecのコア数をhigjspecと同じ2コアにしたところ、insertのスピードはほぼ同等になった
...
indexありの方は全体の件数に関わらず一瞬で返ってきている。これは、indexを貼っているおかげで、対象レコードが1件の中からSELECTを走らせることになっているので、すぐにクエリが返ってくる。

indexなしのクエリに関しては、全件スキャンしてから検索を行っているため、全体の件数が増えるごとにクエリの時間がかかるとともに、全件スキャンの時間はコンテナの性能によるため、基本的にlowspecの方が時間がかかっている。500万件のみlowspecの方が早くなってしまったが、ここの理由に関しては調査しきれていない
#### 詳細ログ
##### High Spec
<details>
